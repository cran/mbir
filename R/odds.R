...
odds<-function(or, p, conf.int){

  if(is.character(or) == TRUE || is.factor(or) == TRUE || is.character(p) == TRUE || is.factor(p) == TRUE){
  ...
  negative<-round(100*(stats::pnorm((log(0.9)-log(or))/abs(log(or)/stats::qnorm(p/2)))),digits = 1)
  positive<-round(100*(1-stats::pnorm((log(1.11)-log(or))/abs(log(or)/stats::qnorm(p/2)))),digits = 1)
  trivial<-round((100-positive-negative),digits = 1)
  LL<-exp(log(or)+stats::qnorm((100-(100*conf.int))/100/2)*abs(log(or)/stats::qnorm(p/2)))
  UL<-exp(log(or)-stats::qnorm((100-(100*conf.int))/100/2)*abs(log(or)/stats::qnorm(p/2)))
